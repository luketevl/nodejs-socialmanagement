  ...
  try {
    ...
    if (entity) {
      ...
      try {
        ...
        const resultCreateSystem = await systemController.create(modelResult, data);
...
const _getUserById = async (_id) => {
  try {
    const entity = await Entity.findOne({ _id });
    if (entity) {
      return { entity, status: true, code: 200, message: 'Entity exist' };
    }
    return { entity: null,status: false, code: 404, message: 'Entity not exist' };
  } catch (err) {
    return {
      entity: null, erro: err, status: false, code: 404, message: 'Entity not exist' 
    };
  }
};
const logout = () => ({
